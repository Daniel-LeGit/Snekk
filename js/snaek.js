export default class Snaek {
    ...
    constructor(scene) {
        ...
        var l, c, color;
        color = 0x004400;
        for (c=0; c<=this.scene.game.config.width-this.tileSize; c+=this.tileSize)
        {
            for (l=0; l<this.scene.game.config.height-this.tileSize; l+=this.tileSize)
            {
                this.scene.add
                .rectangle(c, l, this.tileSize, this.tileSize, color)
                .setOrigin(0, 0);
    ...
    placeApple() {
        ...
        let tail=this.body.slice(0);
        while(tail.some(segment => segment.x===this.apple.x && segment.y===this.apple.y)) 
        {
            this.apple.x = Math.floor(Math.random() * this.scene.game.config.width / this.tileSize) * this.tileSize;
            this.apple.y = Math.floor(Math.random() * this.scene.game.config.height / this.tileSize) * this.tileSize;
        }        
